...
var GE = {
    ...
    init: function() {
        ...
        c(d, function(a) {
            ...
            "/web/20150426064543/http://www.ge.com" == a.origin && (console.log("the event.origin is ", a.origin),
            GE.iframeFlag = !0)),
            GE.eS = a
        }
        , !1),
    ...
    foldTiles: function(a) {
        ...
        setTimeout(function() {
            var a = $("video").height();
            $("#cinemagraph-cont .back").height(585),
...
GE.deepDive = {
    storyId: ["industrial", "supermaterials", "imaging", "diesel"],
    nyNum: 0,
    currentAccordion: 0,
    videoCounter: 0,
    init: function() {
        this.tpl = {
            templates: {},
            loadTemplates: function(a, b) {
                var c = this;
                GE.deepDive.loadTemplate = function(d) {
                    var e = a[d]
                      , f = parseInt(Math.floor(1e4 * Math.random() + 1))
                      , g = "scripts/templates/" + e.toString() + ".html?" + f;
                    jQuery.get(g, function(f) {
                        c.templates[e] = f,
                        d++,
                        d < a.length ? GE.deepDive.loadTemplate(d) : b()
                    }
                    )
                }
                ,
                GE.deepDive.loadTemplate(0)
            },
            get: function(a) {
        ...
        GE.deepDive.tpl.loadTemplates(["hero", "diagram-gallery", "diagram-video", "img-left-half", "img-left-half-img", "img-left-half-sm", "img-right-half", "img-right-third", "locations", "introvideo", "ticker", "video-two-third", "video-two-third-right", "intro", "img-left-two-third", "img-left-third", "full-second"], function() {}
        )
    },
    displayBusiness: function(a, b, c) {
        num = 1 == b ? 1 : 0,
        GE.deepDive.storyOrder = GE.deepDive.storyId.indexOf(a),
        $(".story-wrapper").eq(0).removeClass("next").addClass("current"),
        $(".story-wrapper").eq(num).append("<div class='story " + a + "-deep' id='story-next'></div>"),
        $(".story-wrapper .story-content").eq(0).removeClass("next").addClass("current"),
        1 == b && $(".story").eq(0).attr("id", "test2");
        new heroView({
            model: new Backbone.Model(GE.cities[GE.deepDive.storyOrder].hero[0])
        });
        GE.resize(),
        $(".story").eq(num).append("<div class='story-content' id='story-content-next'></div>"),
        1 == b && ($(".story-wrapper").eq(1).css({
            top: 0,
            display: "block"
        }),
        TweenLite.to($(".story-wrapper").eq(1), .5, {
            css: {
                opacity: 1
            },
            ease: Cubic.easeInOut,
        ...
        $(".story-content").eq(0).attr("id", "test"),
        TweenLite.set($(".story").eq(1), {
            css: {
                rotationX: 0,
                transformOrigin: "0 0"
            }
        ...
        $(".story-content").show(),
        $("#close").fadeIn(200));
        for (var d = (new tickerView({
            model: new Backbone.Model(GE.cities[GE.deepDive.storyOrder].ticker[0])
        }),
        GE.cities[GE.deepDive.storyOrder].modules), e = 0, f = d.length; f > e; ++e)
            for (var g in d[e])
                GE.deepDive.displayModule(g, GE.deepDive.storyOrder, d[e][g]);
        new locationsView({
            model: new Backbone.Model(GE.cities[GE.deepDive.storyOrder].locations[0])
        });
        if (1 == b) {
            $(".story-wrapper.next #story-hero-cont").hide(),
            TweenLite.set($(".story-wrapper.next #story-hero-cont h4"), {
                css: {
                    opacity: 0,
                    marginTop: 20
                }
            }),
            TweenLite.set($(".story-wrapper.next #story-hero-cont h2"), {
                css: {
                    opacity: 0,
                    marginTop: -44
                }
            }),
            TweenLite.set($(".story-wrapper.next #story-hero-cont h3"), {
                css: {
                    opacity: 0,
                    bottom: 30
                }
            ...
            TweenLite.to($("#" + c), 1, {
                css: {
                    bottom: CONFIG.currentH - 220
                },
                ease: Cubic.easeInOut,
                delay: 0,
                onComplete: function() {}
            }),
            TweenLite.to($(".story-wrapper").eq(1), 0, {
                css: {
                    top: 0
                },
                ease: Cubic.easeInOut,
            ...
            $("#story-hero").height(9 * CONFIG.currentW / 16),
            $(".story-wrapper.next #story-hero-cont").show();
            var h = new TimelineLite({
                paused: !0
            });
            h.insert(TweenLite.to($(".story-wrapper.next #story-hero-cont h4"), .5, {
                css: {
                    marginTop: 0,
                    opacity: 1
                },
                ease: Cubic.easeOut,
                onComplete: function() {}
            }), .1).insert(TweenLite.to($(".story-wrapper.next #story-hero-cont h2"), .5, {
                css: {
                    marginTop: -64,
                    opacity: 1
                },
                ease: Cubic.easeOut,
                onComplete: function() {}
            }), .3).insert(TweenLite.to($(".story-wrapper.next #story-hero-cont h3"), .5, {
                css: {
                    bottom: 50,
                    opacity: 1
                },
    ...
    displayModule: function(a, b, c) {
        switch (a) {
        case "diagramGallery":
            {
                new diagramGalleryView({
        ...
        case "videoTwoThird":
            {
                new videoTwoThirdView({
                    model: new Backbone.Model(c)
                })
            }
            break;
        case "videoTwoThirdRight":
            {
                new videoTwoThirdRightView({
                    model: new Backbone.Model(c)
                })
            }
            break;
        ...
        case "intro":
            ...
            break;
        case "imgLeftTwoThird":
            {
                new imgLeftTwoThirdView({
                    model: new Backbone.Model(c)
                })
            }
            break;
